...
//    }
    ...
    @PutMapping
    public ResponseEntity<?> updateHealthProfileParents(
            @PathVariable Integer studentId,
            @RequestBody UpdateHealthProfileDTO healthProfileDTO) {
        HealthProfileResponseDTO response = service.updateHealthProfile(studentId, healthProfileDTO);
        return ResponseEntity.ok(response);
    }

    @GetMapping
    public ResponseEntity<?> getHealthProfileParents(@PathVariable Integer studentId) {
