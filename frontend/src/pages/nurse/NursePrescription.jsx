...
const MedicineDeclarationsList = () => {
  ...
  const checkExpiredSubmissions = (submissions) => {
    const today = dayjs();
    return submissions.map(submission => {
      const endDate = dayjs(submission.endDate);

      // If submission is APPROVED and past end date, mark as EXPIRED
      if (submission.submissionStatus === 'APPROVED' && endDate.isBefore(today, 'day')) {
        return {
          ...submission,
          submissionStatus: 'EXPIRED',
          displayStatus: 'EXPIRED' // Add a display status to differentiate from backend status
        };
      }
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      submissions = checkExpiredSubmissions(submissions);

      // Log để kiểm tra dữ liệu
      console.log('Medicine submissions data after expiry check:', submissions);
  ...
  const getPriorityColor = (record) => {
    ...
    if (record.submissionStatus === 'APPROVED' && endDate.isBefore(today, 'day')) {
      return '#8c8c8c'; // Gray for expired
    }
    if (record.submissionStatus === 'APPROVED' && endDate.diff(today, 'days') <= 2) {
      return '#fa8c16'; // Orange for expiring soon
    }
    if (record.submissionStatus === 'EXPIRED') {
      return '#8c8c8c'; // Gray for expired
    }
    return null;
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (status, record) => {
        ...
        const isExpired = status === 'APPROVED' && endDate.isBefore(today, 'day');
        const isExpiring = status === 'APPROVED' && !isExpired && endDate.diff(today, 'days') <= 2;
